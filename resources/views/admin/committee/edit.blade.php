...
@section('page_title', 'Edit Committee Member')

@section('content')
<div class="max-w-3xl">
    <form action="{{ route('admin.committee.update', $member->id) }}" method="POST" class="space-y-8">
        @csrf
        <div class="bg-slate-950 p-10 rounded-3xl border border-slate-800 shadow-xl space-y-8">
            <div class="space-y-3">
                ...
                <input type="text" name="name" value="{{ $member->name }}" required class="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-900/20 transition-all outline-none">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    <label class="text-xs font-black text-slate-500 uppercase tracking-widest px-1">Role / Designation</label>
                    <input type="text" name="role" value="{{ $member->role }}" class="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-900/20 transition-all outline-none">
                </div>
                <div class="space-y-3">
                    <label class="text-xs font-black text-slate-500 uppercase tracking-widest px-1">Affiliation / Institute</label>
                    <input type="text" name="affiliation" value="{{ $member->affiliation }}" class="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-900/20 transition-all outline-none">
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <select name="category" required class="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-900/20 transition-all outline-none appearance-none cursor-pointer">
                    ...
                    <option value="{{ $cat }}" {{ $member->category == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
            </div>
